...
# start the measure
class AedgSmallToMediumOfficeInteriorFinishes < OpenStudio::Measure::ModelMeasure
  ...
  def name
    return 'AedgSmallToMediumOfficeInteriorFinishes'
  end
  ...
  def arguments(model)
    ...
    lowerPartitions = OpenStudio::Measure::OSArgument.makeBoolArgument('lowerPartitions', true)
    lowerPartitions.setDisplayName('Lower Height of Interior Partition Surfaces?')
    lowerPartitions.setDefaultValue(true)
    args << lowerPartitions
  ...
  def run(model, runner, user_arguments)
    ...
    lowerPartitions = runner.getBoolArgumentValue('lowerPartitions', user_arguments)
    ...
    # partition surface array for later use
    partitionSurfacesFinal = []
    ...
    partitionSurfaces.each do |surface|
      ...
      if !surface.construction.empty?
        ...
        partitionSurfacesFinal << surface
        construction = surface.construction.get
        numLayers = construction.to_LayeredConstruction.get.layers
        insideMaterial = construction.to_LayeredConstruction.get.getLayer(numLayers.size - 1)
    ...
    # lower partition surfaces
    if lowerPartitions
      zValueTarget = OpenStudio.convert(36, 'in', 'm').get
      numChangedPartitions = OsLib_Geometry.lowerSurfaceZvalue(partitionSurfacesFinal, zValueTarget)
    end
    ...
    if lowerPartitions
      runner.registerFinalCondition("#{string} #{numChangedPartitions} furniture surfaces were lowered to 36 inches. #{aedgTipsLong}")
    else
...
AedgSmallToMediumOfficeInteriorFinishes.new.registerWithApplication
